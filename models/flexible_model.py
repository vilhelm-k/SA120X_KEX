...
class FlexibleModel(BaseModel):
    ...
    def build(self):
        ...
        for k in self.K:
            for i in self.V:
                start_minutes = self.tasks.loc[i, "start_minutes"]
                end_minutes = self.tasks.loc[i, "end_minutes"]
                duration_minutes = self.tasks.loc[i, "duration_minutes"]
                self.model.addConstr(self.t[k, i] >= start_minutes, name=f"Earliest[{k},{i}]")
                self.model.addConstr(self.t[k, i] <= end_minutes - duration_minutes, name=f"Latest[{k},{i}]")
